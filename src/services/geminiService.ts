...
export const geminiService = {
    ...
    const response = await ai.models.generateContent({
      ...
      contents: {
        parts: [
          ...
          { text: `${prompt}. TRẢ LỜI NGẮN GỌN, CHIA CÂU NGẮN, DỄ ĐỌC THÀNH TIẾNG.` }
        ]
      },
      config: {
        maxOutputTokens: 200,
  ...
  async analyzeImageStream(base64Image: string, prompt: string) {
    const ai = getAI();
  ...
  async chat(message: string, history: { role: string, parts: { text: string }[] }[]) {
    ...
    const chat = ai.chats.create({
      ...
      config: {
        ...
        - TRẢ LỜI NGẮN GỌN, CHIA CÂU NGẮN, DỄ ĐỌC THÀNH TIẾNG.
