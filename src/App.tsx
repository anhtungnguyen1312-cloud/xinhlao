...
function AppContent() {
  ...
  const handleChatSubmit = async (e: React.FormEvent) => {
    ...
    try {
      let response;
      if (userImg) {
        response = await geminiService.analyzeImage(userImg, userMsg || "Analyze this image.");
      } else {
        const history = chatMessages.map(m => ({
          role: m.role,
          parts: [{ text: m.content }]
        }));
        response = await geminiService.chat(userMsg, history);
      }
      ...
      >
        <AnimatePresence mode="wait">
          ...
          {activeView === 'chat' && (
            ...
            >
              <header 
                className="h-20 border-b border-white/5 flex items-center px-7 shrink-0 backdrop-blur-md sticky top-0 z-10 transition-colors duration-300"
                style={{ backgroundColor: `rgba(15, 23, 42, ${globalOpacity * 0.4})` }}
              >
                <div className="flex items-center gap-3">
                  <div className="w-1 h-6 bg-primary rounded-full" />
                  <h2 className="text-xs font-bold uppercase tracking-[0.3em] text-white/40 hero-title">Hanoi Library AI Assistant</h2>
                </div>
              </header>
              ...
              <div className="flex-1 overflow-y-auto p-7 space-y-5 custom-scrollbar">
                {chatMessages.length === 0 && (
                  <div className="h-full flex flex-col items-center justify-center text-center space-y-8 opacity-20">
                    <div className="w-16 h-16 bg-bg-card/50 rounded-[27px] flex items-center justify-center glass">
                      <MessageSquare className="w-10 h-10" />
                    </div>
                    <div>
                      <h3 className="text-2xl font-serif italic">Tôi có thể giúp gì cho bạn?</h3>
                      <p className="text-sm max-w-xs mx-auto mt-4 leading-relaxed">Tinh chỉnh ý tưởng nội dung, lên kế hoạch chủ đề hoặc chỉ đơn giản là trò chuyện.</p>
                    </div>
                  </div>
                )}
                {chatMessages.map((msg, i) => (
                  <div key={i} className={cn(
                    ...
                    <div className={cn(
                      "w-7 h-7 rounded-[11px] flex items-center justify-center shrink-0 shadow-lg",
                      msg.role === 'user' ? "bg-primary text-black" : "bg-bg-card/50 border border-white/10"
                    )}>
                      {msg.role === 'user' ? <span className="text-[10px] font-bold">U</span> : <BookOpen className="w-4 h-4 text-primary" />}
                    </div>
                    <div 
                      className={cn(
                        "p-4 rounded-[21px] text-sm leading-relaxed border backdrop-blur-xl transition-colors duration-300",
                        msg.role === 'user' ? "bg-primary/10 border-primary/20 text-[#facc15]" : "border-white/5 text-white/80"
                      )}
                      style={msg.role !== 'user' ? { backgroundColor: `rgba(15, 23, 42, ${globalOpacity * 0.33})` } : {}}
                    >
                      <div className="prose prose-invert prose-sm prose-amber">
                ...
                {isChatLoading && (
                  <div className="flex gap-6 max-w-4xl">
                    <div className="w-7 h-7 rounded-[11px] bg-white/5 border border-white/10 flex items-center justify-center shrink-0">
                      <Loader2 className="w-4 h-4 animate-spin text-primary" />
                    </div>
                    <div 
                      className="p-4 rounded-[21px] border border-white/5 text-white/30 text-sm italic backdrop-blur-xl transition-colors duration-300"
                      style={{ backgroundColor: `rgba(15, 23, 42, ${globalOpacity * 0.33})` }}
                    >
                      Đang suy nghĩ...
                    </div>
              ...
              <div 
                className="p-7 border-t border-white/5 transition-colors duration-300"
                style={{ backgroundColor: `rgba(15, 23, 42, ${globalOpacity * 0.4})` }}
              >
                <form onSubmit={handleChatSubmit} className="max-w-4xl mx-auto space-y-6">
                  {selectedImage && (
                    <div className="relative w-16 h-16 rounded-[11px] overflow-hidden border-2 border-primary/30 shadow-2xl">
                      <img src={selectedImage} className="w-full h-full object-cover" />
                      <button 
                        type="button"
                        onClick={() => setSelectedImage(null)}
                        className="absolute top-1 right-1 bg-bg-main/80 rounded-full p-0.5 hover:bg-bg-main text-white"
                      >
                  ...
                  <div className="relative group">
                    <input 
                      ...
                      placeholder="Gửi tin nhắn cho HANOI LIBRARY AI..."
                      className="w-full bg-white/5 border border-white/10 rounded-[21px] py-3 pl-5 pr-20 text-sm focus:outline-none focus:border-primary/50 transition-all shadow-inner"
                    />
                    <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-2">
                      ...
                      <button 
                        type="button"
                        onClick={() => toggleRecording(setChatInput)}
                        className={cn(
                          "p-2 transition-colors",
                          isRecording ? "text-red-500 animate-pulse" : "text-white/20 hover:text-primary"
                        )}
                      >
                      ...
                      <button 
                        ...
                        className="p-2 bg-primary text-black rounded-[11px] hover:bg-primary-hover disabled:opacity-50 transition-all shadow-lg shadow-primary/20"
                      >
          ...
          <>
            ...
            >
              <header className="h-20 border-b border-white/5 flex items-center justify-between px-8 shrink-0">
                <div className="flex items-center gap-3">
                  <div className="w-1 h-6 bg-primary rounded-full" />
                  <h2 className="text-xs font-bold uppercase tracking-[0.3em] text-white/40">Trợ lý THƯ VIỆN HÀ NỘI AI</h2>
                </div>
                <button 
                  onClick={() => setIsChatOpen(false)}
              ...
              <div className="flex-1 overflow-y-auto p-8 space-y-6 custom-scrollbar">
                {chatMessages.length === 0 && (
                  <div className="h-full flex flex-col items-center justify-center text-center space-y-6 opacity-20">
                    ...
                    <p className="text-sm font-serif italic">Tôi có thể giúp gì cho bạn hôm nay?</p>
                  </div>
                )}
                {chatMessages.map((msg, i) => (
                  <div key={i} className={cn(
                    ...
                    <div 
                      className={cn(
                        "p-4 rounded-2xl text-xs leading-relaxed border max-w-[80%] backdrop-blur-xl transition-colors duration-300",
                        msg.role === 'user' ? "bg-primary/10 border-primary/20 text-[#facc15]" : "border-white/5 text-white/80"
                      )}
                      style={msg.role !== 'user' ? { backgroundColor: `rgba(15, 23, 42, ${globalOpacity * 0.33})` } : {}}
                    >
                ...
                {isChatLoading && (
                  <div className="flex gap-4">
                    ...
                    <div 
                      className="p-4 rounded-2xl border border-white/5 text-white/30 text-[10px] italic backdrop-blur-xl transition-colors duration-300"
                      style={{ backgroundColor: `rgba(15, 23, 42, ${globalOpacity * 0.33})` }}
                    >
                      Đang suy nghĩ...
                    </div>
              ...
              <div 
                className="p-5 border-t border-white/5 transition-colors duration-300 bg-bg-main"
              >
                <form onSubmit={handleChatSubmit} className="space-y-4">
                  {selectedImage && (
                    <div className="relative w-16 h-16 rounded-xl overflow-hidden border border-primary/30">
                      ...
                      <button 
                        type="button"
                        onClick={() => setSelectedImage(null)}
                        className="absolute top-1 right-1 bg-bg-main/80 rounded-full p-1 text-white"
                      >
                        <X className="w-2 h-2" />
                      </button>
                    </div>
                  ...
                  <div className="relative">
                    <input 
                      ...
                      className="w-full bg-white/5 border border-white/10 rounded-2xl py-4 pl-6 pr-20 text-xs focus:outline-none focus:border-primary/50 transition-all"
                    />
                    <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-1">
                      ...
                      <button 
                        type="submit"
                        disabled={(!chatInput.trim() && !selectedImage) || isChatLoading}
                        className="p-2 bg-primary text-black rounded-xl hover:bg-primary-hover disabled:opacity-50 transition-all shadow-[0_0_15px_rgba(197,160,89,0.2)]"
                      >
                        <Send className="w-4 h-4" />
                      </button>
                    </div>
